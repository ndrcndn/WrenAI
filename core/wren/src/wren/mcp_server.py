...
def _register_knowledge_tools(mcp: FastMCP, ctx: ServeContext) -> None:
    ...
    def get_context(
        ...
        full content of knowledge/rules/*.md and the views/ and cubes/
        metadata.yml files) when they were indexed. A ``note`` key is present when the
    ...
    def list_knowledge() -> dict:
        ...
        Lists knowledge/ top-level files and one directory level down
        (knowledge.yml, rules/*.md, sql/*.md, ...) — deeper files are not
        served by the resource templates — and reports whether AGENTS.md
